...
def get_price(tickers, start_date, end_date):
    ...
    df = yf.download(tickers, start_date, end_date, progress=False)
    df.reset_index(inplace=True)

    return df


def get_info_df(tickers):
    ...
    columns_dict = {'longName': 'Name',
                    ...
                    'regularMarketDayRange': 'Market Day Range',
    ...
    info_df.loc[['Market Cap. (T)']] = info_df.loc[[
        'Market Cap. (T)']].apply(lambda x: x/1_000_000_000_000)  # Normalise to trillions.
    info_df.loc['Market Cap. (T)'] = info_df.loc['Market Cap. (T)'].apply(
